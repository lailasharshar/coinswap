...
public class SwapExecutorTest {
	...
	public void testBasicStuff() {
		...
		SwapDescriptor swapDescriptor = new SwapDescriptor().setActive(false).setExchange(ScratchConstants.BINANCE).setCoin1("WTC").setCoin2("SALT")
		...
		assertEquals(executor.getBaseCoin(), "BTC");
		...
		Date now = new Date();
		priceData.add(new PriceData().setExchange(ScratchConstants.BINANCE).setPrice(0.0001).setTicker(executor.getCache().getTicker1().getTickerBase()).setUpdateTime(now));
		priceData.add(new PriceData().setExchange(ScratchConstants.BINANCE).setPrice(0.001).setTicker(executor.getCache().getTicker2().getTickerBase()).setUpdateTime(now));
		priceData.add(new PriceData().setExchange(ScratchConstants.BINANCE).setPrice(0.01).setTicker(swapDescriptor.getCommissionCoin() + executor.getBaseCoin()).setUpdateTime(now));
		SwapExecutor.ResponseCode responseCode = executor.swapCoin1ToCoin2(priceData, true);

		SwapDescriptor swapDescriptor2 = new SwapDescriptor().setActive(false).setExchange(ScratchConstants.BINANCE).setCoin1("BTC").setCoin2("BAT")
				.setCommissionCoin("BNB").setDesiredStdDev(1.0);
		...
		priceData.add(new PriceData().setExchange(ScratchConstants.BINANCE).setPrice(0.0001).setTicker(executor2.getCache().getTicker2().getTickerBase()).setUpdateTime(now));
		SwapExecutor.ResponseCode responseCode2 = executor2.swapCoin1ToCoin2(priceData, true);
	}
}